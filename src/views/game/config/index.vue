  ...
  <div class="app-container">
    ...
    <el-table :data="configList" v-loading="loading" border>
      ...
      <el-table-column label="参与人物画像" width="130" align="center">
        <template slot-scope="scope">
          <el-tag :type="portraitEnabled(scope.row) ? 'success' : 'info'">{{
            portraitEnabled(scope.row) ? "是" : "否"
          }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="160">
        <template slot-scope="scope">
          <el-button type="text" size="mini" @click="openDialog(scope.row)"
            >编辑</el-button
          >
          <el-button type="text" size="mini" @click="removeConfig(scope.row)"
            >删除</el-button
          >
        </template>
      </el-table-column>
    ...
    <el-dialog
      :title="form.configId ? '编辑游戏配置' : '新增游戏配置'"
    ...
    >
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="参与人物画像"
          ><el-switch v-model="portraitEnabledValue"
        /></el-form-item>
...
export default {
  ...
  computed: {
    portraitEnabledValue: {
      get() {
        return this.portraitEnabled(this.form);
      },
      set(value) {
        this.form.participatePortrait = value ? "1" : "0";
      },
    },
  },
  created() {
  ...
  methods: {
    getList() {
      this.loading = true;
      listGameConfigs(this.queryParams.category)
        .then((res) => {
    ...
    portraitEnabled(row) {
      return (
        row &&
        (row.participatePortrait === true ||
          row.participatePortrait === 1 ||
          String(row.participatePortrait || "1") === "1")
      );
    },
    openDialog(row) {
      this.form = row
    ...
    submitConfig() {
      ...
      if (
        this.form.participatePortrait == null ||
        this.form.participatePortrait === ""
      )
        this.form.participatePortrait = "1";
      saveGameConfig(this.form)
        .then(() => {
    ...
    removeConfig(row) {
      this.$confirm("确认删除该游戏配置？", "提示")
        .then(() => {
          delGameConfig(row.configId)
            .then(() => {
