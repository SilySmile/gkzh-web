      ...
      <el-tab-pane label="职业与题目" name="questions">
        ...
        <el-table v-loading="questionLoading" :data="questions" border stripe>
          ...
          <el-table-column label="候选状态" width="120" align="center">
            <template slot-scope="scope"><el-tag :type="scope.row.drawCandidate ? 'success' : 'info'">{{ scope.row.drawCandidate ? '候选题' : '普通题' }}</el-tag></template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    loadQuestions() { this.questionLoading = true; listCareerQuestions({ ...this.questionQuery, gameType: 'zycck' }).then(res => { this.questions = res.rows || res.data || []; this.questionTotal = res.total || this.questions.length }).catch(error => this.$modal.msgError(zycckErrorMessage(error))).finally(() => { this.questionLoading = false }) },
    showQuestions(category) { this.activeTab = 'questions'; this.questionQuery.categoryId = category.categoryId; this.loadQuestions() },
    openCategoryDialog(row) { this.categoryForm = row ? { ...row } : { categoryId: null, name: '', description: '', drawMode: 'fixed', sortOrder: this.categories.length + 1, status: '0' }; this.categoryDialogVisible = true; this.$nextTick(() => this.$refs.categoryForm && this.$refs.categoryForm.clearValidate()) },
    ...
    openQuestionDialog(row) { this.questionForm = row ? { ...emptyQuestion(), ...row } : emptyQuestion(); if (!this.questionForm.categoryId && this.questionQuery.categoryId) this.questionForm.categoryId = this.questionQuery.categoryId; this.questionDialogVisible = true; this.$nextTick(() => this.$refs.questionForm && this.$refs.questionForm.clearValidate()) },
    submitQuestion() { this.$refs.questionForm.validate(valid => { if (!valid) return; const category = this.selectedCategory; if (!category) return this.$modal.msgWarning('请先选择有效的职业大类'); const count = this.questions.filter(item => item.drawCandidate && String(item.categoryId) === String(category.categoryId) && String(item.careerQuestionId) !== String(this.questionForm.careerQuestionId)).length + (this.questionForm.drawCandidate ? 1 : 0); const required = category.drawMode === 'random' ? 3 : 1; if (count > required) return this.$modal.msgWarning(this.drawModeText(category.drawMode) + '，候选题不能超过 ' + required + ' 道'); this.saving = true; saveCareerQuestion({ ...this.questionForm, gameType: 'zycck' }).then(() => { this.$modal.msgSuccess('职业题目保存成功'); this.questionDialogVisible = false; this.loadQuestions(); this.loadCategories() }).catch(error => this.$modal.msgError(zycckErrorMessage(error))).finally(() => { this.saving = false }) }) },
